...
int main(int, char **argv) {
    ...
    const std::map<std::set<std::string>, std::vector<std::string>> ext_args = {
        {{"fasta", "fasta.gz", "fastq", "fastq.gz", "bam", "cram"}, {"", "5", "-f \"l>10\"", "-f \"l<10\"", "-f \"l=10\"", "-g"}},
    ...
    const std::map<std::set<std::string>, std::vector<std::string>> file_args = {
        ...
        {{"random2.fastq"}, {"-f 'l>10000' -ql", "-f 'l<9000 & q>10' -ql", "-f 'l>9000 & q>90' -ql", "-f 'q>89' -ql", "-i testFiles/random2.ls", "-e testFiles/random2.ls", "--sample 0.3 --random-seed 42"}},
        {{"random3.bam"}, {""}}
